...
CREATE OR REPLACE FUNCTION public.fetch_user_conversations(p_user_id uuid)
 RETURNS TABLE(id bigint, conversation_id bigint, user_id uuid, partner_id uuid, partner_username text, partner_avatar text, message_text character varying, created_at timestamp with time zone, item_name text, item_image text)
 LANGUAGE plpgsql
AS $function$BEGIN
  RETURN QUERY
  SELECT *
